...
function parseOCRBatterText(text) {
    ...
    for (const line of lines) {
        // 먼저 줄에 타순 번호가 있는지 확인 (1~9로 시작)
        const hasOrder = /^(\d)\s+/.test(line);

        // 영문 포지션 코드 찾기 (1B, 2B, 3B, SS, LF, CF, RF, C, DH)
        let matchedCode = null;
        for (const code of BATTER_POS_CODES) {
            // 포지션 코드는 대소문자 무관하게 단어로 존재해야 함
            const re = new RegExp(`\\b(${code})\\b`, 'i');
            if (re.test(line)) {
                matchedCode = code.toUpperCase();
                break;
            }
        }
        ...
        const korPos = BATTER_POS_MAP[matchedCode];
        if (!korPos) continue;

        // 선수명 추출: 타순 번호 다음 또는 줄 시작부터
        let afterPos = line;
        if (hasOrder) {
            // 타순 번호 제거
            afterPos = line.replace(/^(\d)\s+/, '');
        }

        // 선수명은 보통 한글 또는 영문 (공백 전까지)
        const nameMatch = afterPos.match(/^([가-힣a-zA-Z']+(?:\s*'\d+)?)/);
...
function parseOCRPitcherText(text) {
    ...
    const results = [];
    for (const line of lines) {
        // SP, RP, CP 탐지 (숫자 제거, 대소문자 무관)
        const posMatch = line.match(/\b(SP|RP|CP)\b/i);
        if (!posMatch) continue;
        const basePos = posMatch[1].toUpperCase(); // 'SP', 'RP', 'CP'

        // 내부 저장용 포지션 결정 (SP1~SP5, RP1~RP4, CP)
        let pos = basePos;
        if (basePos === 'CP') {
            pos = 'CP';
        } else {
            // 해당 타입(SP/RP)에서 아직 사용되지 않은 첫 번째 포지션 찾기
            const usedPositions = results.map(r => r.pos);
            const maxNum = basePos === 'SP' ? 5 : 4;
            for (let i = 1; i <= maxNum; i++) {
                const candidatePos = `${basePos}${i}`;
                if (!usedPositions.includes(candidatePos)) {
                    pos = candidatePos;
                    break;
                }
            }
        ...
        const codeIdx = line.toUpperCase().indexOf(basePos);
        const afterCode = line.slice(codeIdx + basePos.length).trim();
        const nameMatch = afterCode.match(/^(\S+)/);
        const name = nameMatch ? nameMatch[1] : '';
        const nums = extractNumbers(afterCode);
        const inputNums = nums.length > 0 ? nums.slice(1) : nums; // ERA 스킵
        const inningRaw = extractInning(afterCode);
        results.push({
            pos,
            name,
            '경기': safeInt(inputNums, 0),
...
async function runOCRPipeline(file, type, day) {
    ...
    try {
        ...
        if (parsed.length === 0) {
            showOCRError('인식된 선수 기록이 없습니다.\n포지션명(포수, 1루수, SP1 등)이 스크린샷에 보이는지 확인해주세요.');
            return;
        }
...
function showOCRConfirmDialog(pairs, type, day, prevRaw, rawText) {
    ...
    pairs.forEach(({ diffRow, todayRawRow }, ri) => {
        ...
        tdPos.className = 'ocr-pos';
        tdPos.textContent = diffRow.pos;
